...
public interface OrderRentRepository extends JpaRepository<OrderRentDTO, Integer>{
	...
	@Query("SELECT dto FROM OrderRentDTO dto WHERE dto.productownerDTO.productownerID = ?1 AND dto.status IN ('PROGRESSING_FAILED','DELIVERY', 'CONFIRMING', 'RENTING', 'PROGRESSING','RETURNING') ORDER BY CASE WHEN dto.status = 'PROGRESSING_FAILED' THEN 1 WHEN dto.status = 'RETURNING' THEN 2 WHEN dto.status = 'DELIVERY' THEN 3 WHEN dto.status = 'CONFIRMING' THEN 4 WHEN dto.status = 'PROGRESSING' THEN 5 ELSE 6 END")
